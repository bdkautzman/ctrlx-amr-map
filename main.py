...
def provide_int32(provider: ctrlxdatalayer.provider, nodeAddress: str,
                   typeAddress: str):
    print("Creating int32  provider node " + nodeAddress, flush=True)
    variantInt32 = Variant()
    variantInt32.set_int32(0)
    return provide_node(provider, nodeAddress, typeAddress, variantInt32)
...
def run_webserver_unixsock():
    ...
    sock_dir = os.getenv("SNAP_DATA") + "/package-run/amr-map/"
