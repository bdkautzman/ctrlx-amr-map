...
class RequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    writeResult = ""
    posArrayValue = ""
    posArrayPath = "cam-builder/position-array"
    ...
    def send_value_response(self, value):
    ...
    def do_GET(self):
        ...
        # HTML
        if self.path.startswith("/amr-map?token="):
        ...
        # if self.path.endswith("/amr-map/x"):
        #     self.send_value_response()
        #     return

        # if self.path.endswith("/amr-map/y"):
        #     self.send_value_response()
        #     return
    ...
    def do_POST(self):
        """do_POST
        """
        # From Cam Builder Code
        # content_length = int(self.headers['Content-Length'])
        # content = self.rfile.read(content_length).decode("utf-8")
        # print("POST", str(content), flush=True)

        # RequestHandler.posArrayValue = content

        # RequestHandler.writePath = RequestHandler.posArrayPath
        # RequestHandler.writeValue = RequestHandler.posArrayValue
    ...
    # def do_PUT(self):
    #     # PUT Requests from client
    #     content_length = int(self.headers['Content-Length'])
    #     content = self.rfile.read(content_length).decode("utf-8")
    #     print("PUT", str(content), flush=True)

    #     content_json = json.loads(content)

    #     if self.path.endswith("/amr-map%2Fx?type=null"):
    #         xVal = content_json.value
    #         print("X: " + xVal, flush=True)

    #     if self.path.endswith("/amr-map%2Fy?type=null"):
    #         yVal = content_json.value
    #         print("Y: " + yVal, flush=True)
